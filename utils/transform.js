...
class DataTr {
    ...
    /** aplpy % caclule value to */
    percentConvert(el) {       

        try {
            var res = (this.value*100)/el.maxValue;
            if(el.appendPercentSimbol == true){
                res = res + "%";
            }
            this.value = res;
        } catch (error) {
